...
latex.DrawLatex(0.05165, 0.072, f"40TW laser, e-laser, #xi = {xi}")

leg = TLegend(0.62, 0.72, 0.87, 0.87)

leg.SetBorderSize(0)
leg.AddEntry(doublet_angles_low_energy, "E_{e^{+}} < 3GeV", "l")
leg.AddEntry(doublet_angles_high_energy, "E_{e^{+}} > 3GeV", "l")
leg.SetFillColor(0)
...
h_frame1 = TH1F('frame', 'energy', 50, -0.0001, 1.1e-3)
h_frame1.GetYaxis().SetTitle("Fraction of counts")
h_frame1.GetXaxis().SetTitle("#delta#theta")
h_frame1.GetYaxis().SetTitleSize(0.05)
h_frame1.GetXaxis().SetTitleSize(0.05)
h_frame1.GetXaxis().SetLabelOffset(0.02)
h_frame1.SetMaximum(0.08)
gPad.SetTicky()
h_frame1.Draw("P")
...
latex.SetTextSize(0.045)
latex.SetTextFont(42)
latex.DrawLatex(0.0, 0.07, f"40TW laser, e-laser, #xi = {xi}")

leg = TLegend(0.65, 0.6, 0.85, 0.75)

leg.SetBorderSize(0)
leg.AddEntry(triplet_angles_low_energy, "E_{e^{+}} < 3GeV", "l")
leg.AddEntry(triplet_angles_high_energy, "E_{e^{+}} > 3GeV", "l")
leg.SetFillColor(0)
leg.SetTextSize(0.05)
leg.Draw()
