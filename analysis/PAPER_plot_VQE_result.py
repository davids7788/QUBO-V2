...
latex.DrawLatex(3.3,.42,"N_{shots} = 512")
latex = TLatex()
latex.SetTextSize(0.045)
latex.SetTextFont(42)
latex.DrawLatex(3.3,.32,"Measurement error mitigation applied")


leg = TLegend(0.4, 0.6, 0.85, 0.9)

leg.AddEntry(ideal_sim_hist, "Ideal Simulation", "f")
leg.AddEntry(fake_nairobi_hist, "fake_nairobi", "f")
leg.AddEntry(ibm_nairobi_hist, "ibm_nairobi", "f")
leg.SetHeader("7 qubit systems")
leg.SetBorderSize(0)
leg.SetFillColor(0)
...
canv.SaveAs("PAPER_VQE_result.pdf")
canv.SaveAs("PAPER_VQE_result.C")
